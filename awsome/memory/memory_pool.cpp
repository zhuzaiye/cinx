// 内存池 https://mp.weixin.qq.com/s/qwv5yIGywFVZB3g8yUyovw
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// 内存池结构体对象
struct mp_pool_s {
  struct mp_large_s *large;  // 大块内存位置
  struct mp_node_s *head;    // 小块内存池头部位置
  struct mp_node_s *current; // 小块内存池现在位置
};
...
struct mp_pool_s *mp_create_pool(size_t size) {
  ...
  int ret = posix_memalign((void **)&pool, MP_ALIGNMENT, size); // 4K +
                                                                // mp_pool_s
  if (ret) {
    return NULL;
  }
  ...
  pool->head->last = (unsigned char *)pool + sizeof(struct mp_pool_s) +
...
void *mp_malloc_block(struct mp_pool_s *pool, size_t size) {
  ...
  unsigned char *ret_addr =
      mp_align_ptr(block + sizeof(struct mp_node_s), MP_ALIGNMENT);
...
void monitor_mp_poll(struct mp_pool_s *pool, char *tk) {
  ...
  for (head = pool->head; head; head = head->next) {
    ...
    if (i == 1) {
      printf("第%02d块small block  已使用:%4ld  剩余空间:%4ld  引用:%4d  "
             "failed:%4d\n",
             i, (unsigned char *)head->last - (unsigned char *)pool,
             head->end - head->last, head->quote, head->failed);
    } else {
      printf("第%02d块small block  已使用:%4ld  剩余空间:%4ld  引用:%4d  "
             "failed:%4d\n",
             i, (unsigned char *)head->last - (unsigned char *)head,
             head->end - head->last, head->quote, head->failed);
    }
  }
  struct mp_large_s *large;
